...
for a in ["softsign", "arctan", "tanh", "sigmoid", "elu", "softsign_shift"]:
    p = f'VQ-CNNI/8/vqc_1_1/{a}/phi_preds.npy'
    pred = np.load(p)
    dd = np.angle(np.exp(1j*(pred - phi)))
    sw = 10*np.log10(dd**2 + 1e-12)
    ps = np.load(f'VQ-CNNI/8/vqc_1_1/{a}/phi_preds_shots.npy')
    ds = np.angle(np.exp(1j*(ps - phi)))
    sshot = 10*np.log10(ds**2 + 1e-12)
    print(f"{a:<16} exact_med={np.median(sw[keep]):7.2f}  shots_med={np.median(sshoot_x):7.2f}")
...
sh = d['swpe_shots_db']
print("shots at i0: per-trial values:", np.round(sh[:, i0], 2))
print("shots median at i0:", np.median(sh[:, i0]), " mean:", np.mean(sh[:, i0]))
print("full-range exact median:", np.median(sw[keep]))
# original VQI-local for comparison
po = 'VQI/8/vqc_1_1_0.7'
phio = np.linspace(-np.pi + np.pi/100, np.pi + np.pi/100, 50)
predo = np.load(os.path.join(po, 'phi_preds.npy'))
ddo = np.angle(np.exp(1j*(predo - phio)))
swo = 10*np.log10(ddo**2 + 1e-12)
i0o = int(np.argmin(np.abs(phio)))
...
if os.path.exists(os.path.join(po, 'phi_preds_shots5000.npy')):
    ...
    d5 = np.angle(np.exp(1j*(s5 - phio)))**2
...
pooled = np.concatenate(all_sw)
print("pooled median:", np.median(pooled))
